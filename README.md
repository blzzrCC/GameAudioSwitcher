...
- **开机切换（新增）**：程序启动（含开机自启）时自动把系统默认输出切到指定设备（耳机 / 扬声器 / 不切换），托盘菜单「开机时切换至」可随时修改 —— 解决开机后 Windows 默认输出设备不对的问题
- **一键扫描设备**：托盘菜单「扫描音频输出设备…」自动枚举本机全部输出端点，按名称关键字自动识别耳机 / 扬声器，可当场「测试切换」，保存后立即生效 —— 新用户不必再去系统声音设置里手工抄写设备名
- **首次运行自动引导**：启动时若配置中的设备在本机对不上，自动弹出扫描向导完成配置；配置文件不存在时由程序自动生成，无需手工准备
...
1. 下载 Release 中的 `GameAudioSwitcher.exe`（或自行构建），双击运行 —— 首次运行会在同目录自动生成 `config.ini`（配置文件）与 `GameAudioSwitcher.log`（运行日志）
2. 程序驻留托盘（耳机图标）
3. 若提示未找到音频设备，在自动弹出的「扫描音频输出设备」窗口中点「自动识别」→「保存并应用」
4. 打开游戏 → 自动切换耳机；关闭游戏 → 自动恢复扬声器

> 提示：右键托盘图标可开关「开机自启」「自动切换」、设置「开机时切换至」，可点「扫描音频输出设备…」重新指定设备、点「立即切换输出设备」或按全局快捷键（默认 `Ctrl+Alt+H`）手动切换；「设置切换快捷键…」可自定义快捷键。
...
startup_switch=speaker              ; 程序启动（含开机自启）时自动切换到的设备
                                    ; speaker=扬声器 / headphone=耳机 / none=不切换
                                    ; 仅在没有游戏运行时执行；可托盘菜单「开机时切换至」修改
...
> `config.ini` 与 `GameAudioSwitcher.log` 均由程序首次运行时自动生成（UTF-8 编码），改动配置文件后需重启程序生效。
...
- **开机切换**：程序启动约 1.2 秒后（设备配置自检通过时）执行一次 —— 若 `startup_switch` 指定的设备可用且当前没有游戏进程在运行，则切过去；已有游戏在跑则跳过，交由边界状态机处理。该逻辑与「自动切换」总开关相互独立
...
- **v1.3.0**（2026-09-22）：新增**开机切换**功能 —— 程序启动（含开机自启）时自动把系统默认输出切到指定设备（耳机 / 扬声器 / 不切换），托盘菜单新增「开机时切换至」子菜单，配置项 `startup_switch`（默认 `speaker`）；运行日志与本机配置不再纳入版本库（`GameAudioSwitcher.log` / `config.ini` 改由程序首次运行时在本地生成）。
- **v1.2.2**（2026-09-15）：应用图标换用高清新版源图重新生成（仍为「耳机 + 双向切换箭头」图案，9 种尺寸），并清理仓库无用/遗留文件（历史图标脚本 `make_icon_variant.py`、上版图标 `app_tray_cat.ico`、对比图 `preview_icon_alt.png`、csc 编译临时文件等）。
