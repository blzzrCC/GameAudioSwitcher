...
- **自定义图标（新增）**：EXE 与托盘图标统一使用头像图案，`app.ico` 含 16/20/24/32/40/48/64/128/256 共 9 种尺寸，由 `make_icon.py` 从源图生成
...
```bat
python make_icon.py "头像.png"      :: 生成 app.ico（9 种尺寸），已生成过可跳过
...
| 脚本 | 用途 |
| --- | --- |
| `make_icon.py` | 源图 → `app.ico`（9 种尺寸，LANCZOS 重采样，非正方形自动居中裁切） |
| `make_icon_variant.py` | 生成备选图标 `app_tray_cat.ico`（小尺寸更清晰的猫咪特写版）与对比预览 `preview_icon_alt.png` |
